...
module.exports = {
  async insertPokemonDataToDatabase() {
    ...
    for (let i = 1; i <= noOfPokemon; i++)
      urlListNames.push(`https://pokeapi.co/api/v2/pokemon-species/${i}/`);
    const pokemonApiFetchName = await Promise.all(
      urlListNames.map((url) => fetch(url).then((res) => res.json()))
    );
    ...
    for (let i = 0; i < noOfPokemon; i++)
      pokemonData.push({
        ...
        generation: pokemonApiFetchName[i].generation.name,
